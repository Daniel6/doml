...
def collectData(data):
    ...
    return np.reshape(X, (len(X), 6)), np.reshape([int(x) for x in Y], (len(X), 1))
...
for i in range(len(trX)):
    # print(predictions[i], trY[i][0], predictions[i]==trY[i][0])
    avg += float(predictions[i]==trY[i])/len(trX)
